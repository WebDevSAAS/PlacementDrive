module.exports = function (app, db) {
  ...
  const getStudentsQuery = (data) => {
    ...
    if (data.id) {
      ...
      if (data.keywords.dob)
        q.dob = new RegExp(".*" + data.keywords.dob + ".*");
      if (data.keywords.branch)
        q.branch = new RegExp(".*" + data.keywords.branch + ".*");
      if (data.keywords.gender)
        q.gender = new RegExp(".*" + data.keywords.gender + ".*");
      return {
        status: "success",
        param: q,
      };
    ...
    try {
      ...
      if (
        ...
        (req.body.accountType === "admin" ||
          req.session.accountType === "mentor" ||
          req.session.userid === k.params.id.usn)
      ) {
